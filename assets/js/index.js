...
search_input.addEventListener('keyup',function(){
    ...
    for(var i=0;i<allCourses.length;i++){
        if(allCourses[i].courseName.toLowerCase().includes(search_input.value.toLowerCase())){
        ...
        deleteSpecificRow_btn[i].addEventListener('click',function(){
            const swalWithBootstrapButtons = Swal.mixin({
                ...
                if (result.isConfirmed) {
                    ...
                    var rowID = Number(this.parentElement.parentElement.querySelector('.rowID').innerHTML);
...
function addEventToUpdateBtnRow(courses){
    for(var i=0;i<courses.length;i++){
        updateSpecificRow_btn[i].addEventListener('click',function(){
            rowID = Number(this.parentElement.parentElement.querySelector('.rowID').innerHTML);
